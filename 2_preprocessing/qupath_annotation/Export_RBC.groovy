...
def saveImageAndMask(String pathOutput, ImageServer server, PathObject pathObject, double downsample, String imageExportType) {
    ...
    def annotations2 = getCurrentImageData().getHierarchy().getDetectionObjects().findAll{it.getPathClass() == getPathClass("RBC: Negative")}
    annotations2.each{
        def roi2 = it.getROI()
    ...
    // Export the mask
    def fileMask = new File(pathOutput, name + '-mask_rbc.png')
